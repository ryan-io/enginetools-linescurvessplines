...
namespace Engine.Tools.LinesCurvesSplines {
	...
	public class Curve2D : Polynomial {
		public Vector2 GetPointQuadratic(float step) {
			var point = Utility.Curve.GetPointQuadratic(_points[0], _points[1], _points[2], step);

			return transform.TransformPoint(point);
		}

		public Vector2 GetPointCubic(float step) {
			var point = Utility.Curve.GetPointCubic(_points[0], _points[1], _points[2], _points[3], step);

			return transform.TransformPoint(point);
		}

		public Vector2 GetVelocityAtPointQuadratic(float step) {
			var point = Utility.Curve.GetFirstDerivativeQuadratic(_points[0], _points[1], _points[2], step);
			var tr    = transform;
		...
		public Vector2 GetVelocityAtPointCubic(float step) {
			var point = Utility.Curve.GetFirstDerivativeCubic(_points[0], _points[1], _points[2], _points[3], step);
			var tr    = transform;
