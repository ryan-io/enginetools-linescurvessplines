...
namespace Engine.Tools.LinesCurvesSplines.Editor {
	public abstract class PolynomialEditor : OdinEditor {
		...
		protected static Quaternion GetRotation(Transform transform)
			=> UnityEditor.Tools.pivotRotation == PivotRotation.Local ? transform.rotation : Quaternion.identity;
		...
		void RemoveParentHandles() => UnityEditor.Tools.current = Tool.None;

		void AddParentHandles() => UnityEditor.Tools.current = Tool.Move;
